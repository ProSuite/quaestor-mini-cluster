	...
	internal class Program
	{
		...
		private static async Task<int> Main(string[] args)
		{
			var parsedArgs = Parser.Default
				.ParseArguments<QuaestorLoadBalancerOptions>(args);
				...
				.ConfigureAppConfiguration(
					(hostingContext, configuration) =>
					{
						...
						string defaultConfig =
							Path.Combine(_configDir, "quaestor.loadbalancer.config.yml");
						string envSpecificConfig = Path.Combine(_configDir,
							$"quaestor.loadbalancer.config.{env.EnvironmentName}.yml");
		...
		private static void ConfigureLogging()
		{
			...
			const string logFileSuffix = "loadbalancer";
