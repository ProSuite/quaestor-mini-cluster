...
namespace Quaestor.Utilities
{
	public static class CertificateUtils
	{
			...
			bool validOnly = true)
		{
			...
			{
				_logger.LogDebug(e, "Error finding using {findType} ({search})", findType,
					searchString);
				yield break;
			}
			finally
			...
			[NotNull] IReadOnlyList<X509Certificate2> foundCertificates)
		{
			...
			{
				_logger.LogDebug("No certificate found.");
				return null;
			}
		...
		private static KeyPair TryExtractKeyPair([NotNull] X509Certificate2 certificate)
		{
			...
			{
				_logger.LogDebug(
					"Certificate {cert} has no private key. It cannot be used as server credentials.",
					certificate);
			...
			{
				_logger.LogDebug(
					"Certificate {cert} is not valid. It cannot be used as server credentials.",
					certificate);

				return null;
			}

			_logger.LogDebug("Trying to extract private key from certificate {cert}...",
				certificate);
		                                       ...
		                                       out string notifications)
		{
			...
			{
				_logger.LogDebug(e, "Error getting private key from certificate.");
